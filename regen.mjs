...
// `doctor`; regenerate alongside the hunks so it can never lie separately.
{
    ...
    fs.writeFileSync(path.join(__dirname, 'docs', 'hooks.json'), JSON.stringify({ generated_by: 'regen.mjs', heads, hooks }, null, 1) + '\n');
    ...
    fs.writeFileSync(path.join(__dirname, 'docs', 'HOOKS.md'), md);
}
