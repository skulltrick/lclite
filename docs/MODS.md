...
| **act on the tile under the cursor without a pick of your own** | the engine's OWN ground pick, one frame deferred: `world.updateMousePicking(px - 4, py - 4)` (exactly what the walk option dispatches) arms `World.click`, the next `renderAll` resolves it into `World.groundX/groundZ`, and a hook after that `renderAll` reads them, then sets both to `-1` and calls `World.clearPick()` — before `gameLoop()`'s own walk consumption at the top of the next tick can walk you there. Costs ZERO `World.ts` hunks and cannot drift from the walk pick (`mods/tile-markers`). Use true-tile's per-frame `hoverArmed` pick instead when you need the tile EVERY frame: arming the engine's pick per frame would overwrite a real walk click's pixel. |
...
| highlight the tile under the mouse | `World`'s OWN ground pick: `renderQuickGround()` / `renderGround()` test `World.click` + `insideTriangle` per front-facing ground triangle while drawing back-to-front, so the last hit is the nearest tile. Arm your own pixel beside it (`mods/true-tile`'s hovered marker: fields by `groundX`, three pick sites, arm in `gameDrawMain` before `renderAll`, draw after `coordArrow`). Record the level the rasterizer DREW (its `level` argument = the square's `originalLevel`) and sample `World.groundHeight()` — `getAvH` promotes a LinkBelow level a second time and lifts the outline 240 units off a bridge deck |
| highlight the tile you are walking to | the client's OWN destination: `Client.minimapFlagX/Z`, which `tryMove()` sets to the path's destination tile (the nearest REACHABLE one when the click was blocked) and the engine zeroes when the model arrives (0 = none). Arm it once per frame and draw it as ground geometry on that tile's own turn in `fill()`, like the true tile (`mods/true-tile`'s destination marker) |
...
| **run a chat command from the page** (a command palette / staff tool) | the chat send path in `Client.handleInputKey()` — the `else if (this.chatModalId === -1)` guard and the branch it opens. Write the command BODY into a terser-reserved window slot, queue an Enter, widen that guard for the one tick the armed command arrives on, fill `this.chatInput` with `'::' + body` and let the engine's OWN send run below (`mods/rotten-potato`). A page-only version that types the command with synthetic keystrokes is a **silent no-op while a chat modal is open** — which is exactly the state a quest dev is in — and `give`/`setstat`/`advancestat` all work server-side with a dialogue up |
...
| click an interface component (a button, a dialogue option) | **`doAction()` — never build the packet yourself.** A real click's row is built by `addComponentOptions()` and dispatched by `mouseLoop()` → `doAction(i)`; so a mod writes the row it wants into the menu arrays at `menuNumEntries` and calls `doAction()` on it, then clears the row: `menuAction[slot]` = the component's own action (`MiniMenuAction.IF_BUTTON` for a `BUTTON_OK` row, `PAUSE_BUTTON` for a `BUTTON_CONTINUE` "click here to continue" — 997, still below `_PRIORITY` 2000 so `doAction`'s priority strip leaves it alone) and `menuParamC[slot]` = the component id (`mods/hotkeys`'s `hotkeysClickComponent`). That is what makes the click identical to a mouse click: `clientButton(com)` still runs for a `clientCode`, the pause guard still applies (`resumedPauseButton` — the flag that also makes the button read "Please wait..."), and no mouse coordinate is faked, so the click is honest to the client's own input tracking — which the camera mod has stopped reporting at all (`mods/camera/README.md`). Component ids come from the interface you already have open (`IfType.list[chatModalId].children` → `buttonType`/`buttonText`), which is why a mod should recognise the interface by SHAPE rather than by a hardcoded id. **A component's own `x`/`y` are ALWAYS 0** — the interface stream never carries them: the packer writes a child's `.if` `x=`/`y=` into the PARENT's `childX[]`/`childY[]` (`engine tools/pack/interface/PackShared.ts`, and the unpacker reads them back from there), so a child's real offset is `parent.childX[i] + child.x` — which is exactly how `drawInterface` and `addComponentOptions` compute it. Any SHAPE-based rule (the option-row policy, a hover test, a synthetic hit test) must use that or every row collapses onto one point; `mods/hotkeys` lost its number row to precisely this |
...
`mods/tcg`, `mods/true-tile`). Two rules that bite: the client's tsconfig has
...
arms engine state and reads the result back (the true-tile mod's hovered marker: arm the
pick before `world.renderAll`, draw after it) inserts at both ends of the same call sequence — keep
...
only ever match the revision it was authored on — `mods/true-tile`'s payload import
...
candidate and reports which patch JSONs it churns; `mods/tile-markers/tools/
tile_markers_site_search.py` is the one-file version of both — insert via the mod's own
edit script with `PARK_ANCHOR=<line>`, run a real `regen`, report everything `git status`
names outside your own mod — and its measurement is worth knowing: **the END of the class
is not a safe home** (it churned ground-items and true-tile), and neither is any boundary
within a few lines of another mod's parked block (MIXED MARKERS)). `mods/world-map/README.md`
§The parked-block landmine has the measured results.
...
- [x] anti-cheat/telemetry suppression — `mods/anti-cheat` (split out of camera 2026-09-09) — **FOLDED BACK INTO CAMERA 2026-09-20: the mod, its toggle and its `antiCheat` key are gone.** The camera mod now never sends the legacy telemetry at all (see `mods/camera/README.md`): the mouse-move position packer, `EVENT_MOUSE_CLICK`, `EVENT_APPLET_FOCUS`, `EVENT_CAMERA_POSITION` and all 16 `ANTICHEAT_CYCLELOGIC*`/`OPLOGIC*` counter packets are removed from the client, unconditionally — the mod's own on/off is the only control, and the authentic behaviour is what you get with it off. Why: the old shape was a mod in the list whose NAME was an anti-cheat evasion toggle and whose default was "authentic" (it did nothing until you touched the row), which is the worst of both — a bad look that also did not do the thing. 254 loses the suppression with it (camera is not available there). **MIGRATION:** an install that already has the old guards applied must be reset to pristine once before re-applying — the deleted corpus cannot reverse-apply them, so the folded hunks would report `anchor not found` (see `mods/camera/README.md` §Migrating).
...
- [x] **Hotkeys (F-key tabs, Esc closes interfaces, WASD camera) — SHIPPED 2026-09-17** (`mods/hotkeys`, TYPE B + TYPE A panel rows). Design port of OSRS's own shortcut table + RuneLite's Key Remapping. ENGINE: two no-op hooks (`hotkeyKeyDown`/`hotkeyKeyUp`) inserted into `GameShell.onkeydown`/`onkeyup` before the engine's key queue — a claim means `preventDefault` + return, so a bound key can never also type into the chatbox; the decision itself is PURE (`files/webclient/src/client/Hotkeys.ts`, 94-check bun harness `mods/hotkeys/tools/hotkeys_test.ts`) and Client applies its action (TAB / CLOSE / CLEAR_CHAT / CAMERA / CLAIM). F-keys switch the sidebar exactly like clicking an icon (`activeIcon` + `redrawSide` + `redrawIcons`), and a tab key closes an open SIDE interface first (`closeModal()`) because in this rev the bank/shop own the sidebar (`drawSide` prefers `sideModalId`) — otherwise the key looks dead. Defaults = OSRS for the tabs this rev has (F1 combat … F6 spellbook, F8 friends, F9 logout, F10 options, Esc inventory); F7/F11/F12 and Ignore/Controls/Music stay free — **F11/F12 are the browser's** (fullscreen/devtools; a page cannot block them) and slot 7 has no interface in 289 at all. Esc = discard half-typed chat line → leave typing mode → `closeModal()` (the X button's own path; server `CloseModalHandler` → `requestModalClose` → `Player.closeModal()` runs `if_close`) → else its bound tab; **interface 3559 (the character-design screen) is exempt** via the `CC_DESIGN_PREVIEW` client code (the only interface of 11,942 carrying it) since the tutorial soft-locks without it. WASD sets `keyHeld[3|4|1|2]` — the arrow keys' own per-frame path, so easing/pitch clamp/camera telemetry are untouched — with RuneLite's **"Press Enter to Chat..."** chat lock (drawn in `drawChat()`) because this client's chatbox is always live; the whole mod stands down while `Client.ingame` is false so the login fields keep their keys. PANEL: 21 rows (4 toggles + 13 tab keys + 4 camera keys, hand-mirrored from the core and asserted equal by the harness) and one cross-mod courtesy: the panel yields F1 to the game while the canvas has focus and a tab is bound to it (`hotkeysBinding('F1')`), FAB still opens it. `panel.js?v=7`. **ADDED 2026-09-19 — dialogue keys** (`hotkeysSpace` + `hotkeysNumbers`, both on): **Space** presses "Click here to continue" and **1–5** pick an option in a "Select an Option" menu — the OSRS/RuneLite behaviour this client lacked. Both go through `doAction()` — the engine's OWN click dispatch — via `hotkeysClickComponent(comId, action)`, which builds the menu row for the call and tears it down after it, so the client-code hook (`clientButton`), the packet and the one-click pause guard are IDENTICAL to a mouse click: PAUSE_BUTTON → `RESUME_PAUSEBUTTON` for a continue button, IF_BUTTON + the option's component id for an option (which is what sets the server's `last_com`, and the dialogue procs `switch_component (last_com)` on it — `interface_chat/scripts/chat.rs2`). **No mouse coordinates are faked**: the client's own input tracking stays honest, which a synthetic-click approach could not promise. WHICH interfaces are numbered lists is decided by SHAPE, not by an id (`hotkeysOptionRows`: 2–5 plain buttons, one per line — distinct y, no overlap — sharing a left edge), because the rev's lookalikes must keep their keys: `skill_multi*` stacks four buttons on ONE line per item and puts the items side by side, and `multiobj*` is a row of equal-y cells; both are refused by the per-line rule. `hotkeysChatRoot()` is the interface `drawChat()` draws (`chatModalId`, else `tutComId`), and the tutorial's buttonless "Click to continue" line (`tutComMessage`) is cleared exactly as the engine's own left-click clears it. The dialogue keys are checked BEFORE the tab bindings, so a digit bound to a tab picks the option while a dialogue is up and opens the tab once it is gone; a live chat line still wins over both. Harness 94 → 159 checks (the option-row policy is re-derived from the rev's own `.if` files, and the core's spelled-out `HOTKEYS_BUTTON_OK`/`CONTINUE` are asserted equal to `IfType.ts`'s `ButtonType`). PANEL: 23 rows (`hk-space`, `hk-numbers`). `panel.js?v=27`. **FIXED 2026-09-19 — the number row was dead on arrival**: the row extraction fed `child.x`/`child.y` into the shape policy, but those are always 0 (see the component-geometry fact in the hook table above), so all five option rows collapsed onto one point, `hotkeysOptionRows`' per-line rule refused the list, `dialogueOptions` stayed 0 and the digits were never claimed — while Space kept working, because the continue-button check needs no geometry. The extraction moved into the core (`hotkeysRows(com, lookup)`, a row's offset = `parent.childX[i] + child.x`) so the harness can hold the PACKED shape against the policy instead of the `.if` text, and it now asserts both that a live `multi5` yields five rows and that the old reading is refused. Harness 159 → 171.
- [x] **Hover tile (RuneLite Tile Indicators, hovered tile) — SHIPPED 2026-09-18, RETIRED 2026-09-21: FOLDED INTO `mods/true-tile`.** Its code, its three `hoverTile*` keys and its harness moved there as that mod's *Hovered tile* section (the panel's `kind:'section'` row kind exists for it), so the standalone folder is gone and the merged design is documented in `mods/true-tile/README.md`. What the entry leaves behind as law: (1) **the engine's own ground pick IS the hover pick** — arm a second pixel beside `World.click` in `renderQuickGround`/`renderGround` and the last hit (tiles draw back-to-front) is the tile a click would walk you to, with the engine's own visibility for free; (2) a marker drawn OVER the scene (after `renderAll`, before `areaGame.draw`) is a cursor highlight and is deliberately not occluded, unlike a ground decal; (3) **record the level the rasterizer DREW** (its `level` argument = the square's `originalLevel`) and sample it raw — `getAvH` promotes LinkBelow levels, which is right for "how high is the player standing" and wrong for "how high was this ground drawn"; (4) a payload's keys belong in the payload's own validator, and the shipped bundle keeps those string literals verbatim, so grepping the deployed client.js for them is the prod-side proof that the panel and the engine agree.
...
- [x] **GPU renderer (RuneLite equivalent) — SHIPPED 2026-09-10 as v1 (WebGL2), rewritten the SAME DAY as v2 (WebGPU) which is what ships today.** The long paragraph below is the **v1 design record** (its `gl_FragDepth`/`MAX_TRIS 98304`/full-buffer-upload details are v1-only); **read the v2 paragraph at the END of this entry first** — the file headers in `mods/gpu/files/` are also v2. v1 Design: CPU keeps ALL engine logic (culling/projection/lighting/picking untouched — `Model.mouseCheck` and `World.groundX` resolve exactly as before because they run before rasterization); the ONLY hijacked seam is the three `Pix3D` triangle entry points (`gouraudTriangle`/`flatTriangle`/`textureTriangle`), patched at module load via a single side-effect import hunk in `Client.ts` (line 1, ~87 pristine lines clear of the next mod's hunk). Captured triangles replay on the GPU in **capture order via `gl_FragDepth`** (GEQUAL, clear 0) = the painter's algorithm preserved exactly, blend mirrors `Pix3D.trans`'s integer mix (`ONE_MINUS_SRC_ALPHA/SRC_ALPHA` with `a=trans/256`; `trans=0` stays a plain overwrite), so translucent water/glass land where software put them. Texture sampling reproduces the rasterizer's affine plane bit-for-bit: same `<<14/<<8/<<5` int32 coefficients, `w>>14` (lowMem `>>12`) fixed-point division, `16256/4032` clamps, `0x3f80/0xfc0` row masks, `0xf8f8ff` 4-band R32UI texel array + the `(e>>21)&3`/`(e>>23)&31` shade-band extraction (e = `(shade<<17)|0`, textureRaster's accumulated word); holes `discard`, holeless-zero paints black — `textureRaster` has NO alpha branch (verified), replace always. Per-run scissor reproduces `Pix2D.setClipping` (headicon/label clips included). **Everything else stays software**: item icons, minimap, sidebar, chat, login/title — and the Pix2D overlay writes INTO the game buffer (bubbles/hitbars/trackers/orbs) are uploaded each frame as an R32UI texture and composited last, non-black = replace, which is exactly software's ordering (world first, overlays after) and what makes camera/xp-drops/stat-orbs pixel-identical with GPU on — zero coupling. Settings: `localStorage gpu` (default off), refreshed at `Pix2D.cls()` once per frame (never flips mid-frame; no `applyCameraSettings` hook needed). Self-disables + auto-falls-back on missing WebGL2/shader/link failure/context loss/`gl.getError`/triangle overflow (overflow frame stays correct: leftover tris ride the overlay). v1 honest gaps (also in file header): HSL shade steps per-pixel on GPU vs per-8px software (same look — RuneLite's GL shader made the same call), lowDetail's 4px blocks are a CPU artifact GL doesn't emulate, pure-black overlay pixels let terrain show through. `tools/gpu_parity_test.ts` (bun) rasterizes random tris through the REAL software rasters and a JS mirror of the shader: flat & textured paths EXACT (0 value mismatches), gouraud diffs traced to synthetic-colour-table HSL straddling + fill-rule edge jitter (GL diamond vs scanline top-left), not shader math. Tunables: `MAX_TRIS` 98304 (18.9 MB). Panel: Rendering group 'GPU rendering' toggle. WINDOWS ADAPTER HINT (2026-09-16): the adapter is requested BARE on Windows — Chromium ignores `powerPreference` there and logs a console notice for every optioned call (crbug.com/369219127), so `GpuContext.prefersBareAdapter()` (gated off `navigator.userAgentData?.platform ?? navigator.platform`) keeps the console clean while the high-performance hint stays on the platforms that honour it (dual-GPU Macs); the bare-retry fallback that Brave's fingerprinting defenses need is unchanged. **v2 (WebGPU) — WHAT SHIPS NOW, shipped 2026-09-10 → 09-13, rewrite decision + probes in `docs/archive/gpu-v2-assessment.md`:** native `navigator.gpu` + WGSL from TypeScript, no Rust/wgpu/WASM. Four files in `mods/gpu/files/webclient/src/gpu/`: `GpuFormat.ts` = the backend-neutral CAPTURE CONTRACT (the shared tier of the plan's split diagram: stride `VS*4` = 64 B, slots `[0..1]` xy already projected by the CPU, `[2]` shade word, `[3]` mode, `[4]` seq, `[5]` alpha = `Pix3D.trans/256`, `[6..11]` u,v,w,texId), `GpuShaders.ts` = the WGSL, `GpuContext.ts` = the whole adapter→device→overlay-canvas→configure lifecycle, `GpuRenderer.ts` = capture + pipelines + composite (the only file the `Client.ts` import hunk pulls in; the other three come with it). **What v2 buys:** ONE scene draw + ONE HUD overlay draw per frame instead of v1's 30-60k `drawArrays` — painter's order preserved by carrying the capture sequence number in the vertex's depth slot (`z = seq * 2^-23`, exact in f32 to 8.4 M tris) against a `depth32float` cleared to 0 with `greater-equal`, so "later capture always wins a pixel" is resolved by the fixed-function depth unit; WebGPU has no per-draw scissor, so v1's clip-rect batching (the exact fragmentation that forced the draw count) is gone and ordering carries correctness instead. **Compositing (differs from v1 AND from the abandoned WIP — deliberate):** the WebGPU canvas is a real DOM element positioned over the game rect, `alphaMode:'opaque'` — a detached or `display:none` WebGPU canvas may never be PRESENTED (Dawn only presents what the page renders), which makes a `drawImage` readback read an empty buffer, the suspect the WIP's `gpudbg` experiments never ruled out because every debug mode shared that path; with a visible canvas there is no presentation variable at all. When the GPU owns a frame the patched `PixMap.draw` SKIPS `putImageData`; on every non-GPU frame the overlay hides and software resumes pixel-exact. GPU frames clear the game buffer to a sentinel no engine pixel can hold (`SENTINEL` = `1 << 24`; the 0 → 1 → bit-24 history and why are "Sprite black vs the sentinel" in `mods/gpu/README.md`) so that neither real `Colour.BLACK` HUD pixels (minimenu title bar/shadow, text shadows) NOR the engine's sprite-black (1 — item-icon outlines, interface graphics, the click crosses) are discarded by the overlay. **P5 (2026-09-11) — real texel sampling on GPU:** `captureTexTri` ports v1's parity-tested affine plane (same `<<14/<<8/<<5` int32 coefficients walked from `originX/originY`), the texel pool uploads as a 50-layer 128×512 `texture_2d_array<u32>` mirroring `getTexels` (4 lightness bands, `0xf8f8ff`, 64 px upsample, lowMem layout variant, `pushTexture` dirty-tracking), the WGSL fragment stage mirrors v1's GLSL verbatim (`w>>14`/`>>12`, `0x3f80/0xfc0` masks, `16256/4032` clamps, holes `discard`, opaque-black replace, no alpha mix), `texHoles` stays authoritative. **P7 (2026-09-13):** dirty-rect HUD upload (scan the non-sentinel bbox, `writeTexture` that sub-rect, skip empty frames) + honest perf stats (`window.lcliteGpuStats` = tris / batches / cpuMs / hud px, what the panel row prints) + fullscreen reparent for the DOM overlay. **Failure model (unchanged from v1's contract):** every init/validation/device-lost/error-scope path disables the GPU, flips `localStorage 'gpu'` off and surfaces the reason VERBATIM in `window.lcliteGpuError` (the panel row prints it) — the software renderer underneath is never touched. Settings: `localStorage 'gpu'` (default OFF), re-read at `Pix2D.cls()` once per frame so it never flips mid-frame; `localStorage 'gpudbg'='1'` draws the P1 proof triangle through the same canvas/place/depth plumbing (one glance separates "backend/canvas broken" from "capture/depth wrong"). Tunable: `MAX_TRIS` 65536 (12.6 MB capture; a busy 289 scene runs 15-30k). Panel: a TOP-LEVEL `GPU` row (not the old Rendering-group toggle) with a live status line (`<tris>△ · <calls> calls · <ms>ms`, `starting…` until the first frame, `off: <reason>` on failure) and the master key `gpu`. Harness: `mods/gpu/tools/gpu_parity_test.ts` was ported to the v2 API (`captureTri`/`captureTexTri`/`nv` reset) and reports the SAME results as the v1/WebGL2 baseline (gouraud big 7729 / maxd 246, flat 0/0, tex d1 15); the residual mirror-vs-software deltas are v1's own documented 16.16 fixed-point interpolation-stepping artifact, not a v2 regression. **PIXEL SCALING MOVED INTO THE MOD (2026-09-19):** the mod's panel section now carries the one setting it owns — `gpuPixelScaling` (`'pixelated'` default | `'auto'`, shown as **Pixel scaling**: Pixelated (recommended) / Auto) — and the mod owns `#canvas`'s inline `image-rendering` with it: written at `attach()`, again on `window` `load` (the body's own `loadSettings()` runs after the module and would otherwise clobber it from the page's legacy `filtering` key), then every frame at `Pix2D.cls()`; with the GPU off (or self-disabled) it is forced back to `'auto'`, so pixel scaling is a GPU feature by construction. It replaces the LCLite core row, which wrote the page's `filtering` key and displayed "Pixelated" while the page actually booted `auto` (smooth) — a panel row's `def` is a display fallback, never an applied default. The overlay's existing `image-rendering` copy in `place()` is what keeps the game rect scaling exactly like the sidebar, chatbox and minimap beside it.
- [x] **World map (the client's own OSRS-style map) — SHIPPED 2026-09-22** (`mods/world-map`, TYPE B in TWO bundles + a page-script overlay). A globe orb on the minimap panel, immediately left of wiki-lookup's orb: click it and the client's own world map app fills the window — terrain, `labels.dat` place names, `mapfunction` POI icons with their key, npc/obj spawn dots, multi-combat zones, the main/dungeon/extra sheets, zoom/pan, and a **you-are-here** marker that follows the player and switches sheet when they are underground. THE FINDING THAT SHAPED IT: `webclient/src/mapview/MapView.ts` (a standalone `GameShell`, ~1960 lines) and `WorldMapFont.ts` are ALREADY in the client and ALREADY built by `bundle.ts` (it is one of the three entrypoints), and the engine ALREADY packs and serves its data (`tools/pack/map/Worldmap.ts` → `/worldmap.jag`, a debug-only route) — but NOTHING loads the app: no page, no button. So the mod is wiring, not a port: 4 hunks in Client.ts (the orb: settings, box, hit test, click, pixels, page bridge), 4 in MapView.ts (the marker + the page's ONE reserved entry point `worldMapUi(cmd)`, and a REPLACE that makes `loadWorldmap` read a URL the page chose), 1 in client.ejs (the script tag). TWO BUNDLES, ONE PAGE: the map app takes `#canvas` at module load and runs its own mainloop, so it CANNOT live in the game's page — the mod's page script builds a fixed div sized to the canvas rect (scale/letterbox included, z 9400: above the canvas, below the FAB/panel) and runs the map in an IFRAME inside it, at the canvas's own scale. Cross-realm names are reserved in bundle.ts and every crossing value is a POSITIONAL ARRAY (`worldMapToggle`, `worldMapPlayer` = `[x, z, level, valid]`, `worldMapUi`, `lcmWorldMapBounds`). WORLD TILES, NOT SCENE TILES: the client keeps entities in scene coordinates, so the bridge adds `mapBuildBaseX/Z` (the hint arrow's own conversion) — without it every player reads as tile (54, 50) and lands on no sheet. THE DATA: the mod's page probes `/worldmap.jag` then `/client/worldmap.jag` (a static copy `lclite.mjs build` deploys from the install's own packed jag, which is also what makes the map work through the launcher's join bridge) BEFORE booting the app, because `loadWorldmap()` retries forever and never checks `response.ok` — a 404 would hand it an HTML error page. THE DIFF-ALIGNMENT LANDMINE (cost the most time, now documented in the mod's README): the mod's 146-line parked block in Client.ts changes how `git diff -U0` aligns the WHOLE file, and at several sites it split seven of ground-items' islands into marker-less fragments that regen then filed under `camera` (`doctor` exits 3). The clean sites were found by brute force (`tools/wm_bisect3.py`): `before gameLoop()` / `gameDrawMain()` / `getSpecialArea()` are clean, `before minimapDraw()` / `drawSide()` / `drawChat()` are not. Also learned: `apply` writes `installed.json` from the mods it can VERIFY (a hunk's replace text must be in the tree), so a tree edited before its regen makes the mod vanish from the manifest and the panel — the order is always edit → regen → apply. Harness `mods/world-map/tools/world_map_test.ts` (141 checks: both cores' pixels included, the area table re-derived from MapView's own `reloadMain`/`reloadDungeon`/`reloadExtra` and asserted equal), the idempotent `tools/world_map_edit.py`, `lclite.mjs build` extended to deploy `mapview.js` + the jag. 274 inherits (142/142 exact); not ported to 254. `panel.js?v=33`.
- [x] **Tile markers (RuneLite Ground Markers) — SHIPPED 2026-09-22** (`mods/tile-markers`, TYPE B, 5 hunks in ONE file). **Shift+right-click a tile and the right-click menu gains a "Mark tile" / "Unmark tile" row, and that row is what toggles the mark** — RuneLite's own interaction, ported (their `onMenuEntryAdded` appends the same row while Shift is held AND the menu entry being built is the WALK option); the marks persist in localStorage, draw only on the plane they were made on, and are culled at RuneLite's own `MAX_DRAW_DISTANCE` of 32 tiles. THE DESIGN DECISION: the tile comes from the ENGINE'S OWN ground pick — `world.updateMousePicking(px - 4, py - 4)` is exactly what the walk option dispatches, so the marked tile is the tile a click at that pixel would have walked you to, and the mod needs no `World.ts` hunk at all (which also keeps it out of the three pick sites `true-tile` owns). That pick resolves during the NEXT `renderAll`, so the click only ARMS it and a hook between `removeSprites()` and `entityOverlays()` reads `World.groundX/groundZ` back, then clears them and calls `World.clearPick()` before `gameLoop()`'s walk consumption can see them — **the mark never walks you**. THE ROW GOES INTO THE MENU THE ENGINE ALREADY OPENED, and the frame order is why: the menu is BUILT every frame while closed (`otherOverlays()` calls `buildMinimenu()` only when `!isMenuOpen`), FROZEN when `openMenu()` opens it, and PAINTED later in that same frame by `drawMinimenu()`. So the row cannot be written at build time (the tile is not known yet) nor at open time (that happens in `mouseLoop`, before the draw) — it is inserted by the same post-`renderAll` hook, at slot 1 (the bottom option, above 'Cancel'), and `openMenu()` is called AGAIN for the layout (it is pure layout: width/height/x/y off the option list). The tile rides the row's own `menuParamA/B/C`, so row and tile cannot drift apart, and the click is dispatched at the top of `doAction()` — the funnel wiki-lookup uses — stamped `TM_MENU_ACTION` 1235 (>1000 so the menu's own bubble sort cannot move it, < `_PRIORITY` so it is never priority-wrapped, colliding with no engine action id). **The row is only added when the menu holds a WALK option** (RuneLite's own condition), which is also what keeps it from ever being the left-click default. Divergences from RuneLite, documented in its README: the tile is the one under the cursor at the CLICK, not a live per-frame hover pick (arming the engine's pick every frame would fight the real walk click, and true-tile owns the pick sites that make a hover pick safe), and the row stays in the frozen menu after Shift is released; no import/export, no labels, no per-marker colours; absolute world tiles in ONE key instead of their per-region config keys. The store is a JSON array of FLAT `x, z, level` triples — positional, never objects, because the property mangler renames object-literal keys. Settings: master `tileMarkers` (default ON — an install that never opened the panel still gets the feature), `tileMarkersColor` #ffff00 (their `markerColor`), `tileMarkersFillColor` #000000 (their fill is always black), `tileMarkersOutline` 2px, `tileMarkersFill` 20% (their 50/255), plus a **Clear all markers** action row that deletes the store key. Decal = `true-tile`'s mitred ring + wash as Pix3D triangles carrying `Pix3D.trans` (drawn OVER the scene, like their `ABOVE_SCENE` overlay; the payload keeps its own copy of the geometry — mods are independent, a shared library would make one mod's hunk a dependency of the other's). Harness `tools/tile_markers_test.ts` (155 checks; the decal is verified by rasterizing the emitted triangles against an independent geometry oracle — border band opaque, interior wash-only, nothing outside the quad, plus the triangle budget and the thin-tile fallbacks; the menu row's label and action id are checked too). `panel.js?v=35`. 274 inherits (147/147 exact); not ported to 254. **The parked-block site is a measurement**: the end of the class churned ground-items' and true-tile's corpora, and the boundaries beside their parked blocks reported MIXED MARKERS; `tools/tile_markers_site_search.py` found the clean one (before `getOverlayPosEntity()`) and is the reusable pattern for the next mod.
- [x] **Rotten Potato (a staff command palette) — SHIPPED 2026-09-23** (`mods/rotten-potato`, TYPE B + a page layer). Click the potato in the page corner and a searchable list of every command THIS world has opens — the engine's own `::` commands and the content repo's `::~` debug procs — so spawning items, levelling an account, jumping to a quest step or driving an interface is two clicks instead of typing a command you half remember. 209 entries (171 debug procs + 38 engine/client commands) in the six tabs (`::~help`'s five + the prod-gated staff set), favourites/recents, per-command arg forms with item NAMES (spaces become underscores), and a Raw tab. THE DESIGN DECISION, and the reason it is not page-only: the page writes the command BODY into the terser-reserved window slot `rottenPotatoCmd` and queues an Enter, and ONE Client.ts island widens the `chatModalId === -1` guard for that tick and fills the engine's own `chatInput` with `'::' + body` — so the send is the engine's own path (::fpson, ::tcg, colour prefixes, CLIENT_CHEAT included) and it works with a dialogue OPEN, which typing into the chatbox cannot do. Measured in game: `::~kbd` and `::~varrock` both ran from the palette, the second one with a message box up. TWO Enters, on purpose: with hotkeys' chat lock on the first flips its typing flag and the second flips it back (hotkeys never claims Enter, so both reach the engine). NO parked block at all (one island in Client.ts, one reserve in bundle.ts, one tag hunk in client.ejs) — the parked-block diff landmine cannot bite, verified by a real regen naming nothing outside the mod. Harness `tools/rotten_potato_test.mjs` re-derives the catalogue from `ClientCheatHandler.ts` + the content repo's `[debugproc,…]` declarations and asserts every entry exists with the same arg count (2226 checks).
