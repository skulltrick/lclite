...
credits HUD (top-right of the game frame, or `::tcg` in the chatbox): five
...
- `files/webclient/src/tcg/tcg_core.ts` — the economy engine: xp→credit
  ...
  collections).
- `files/engine/public/lclite/tcg/ui.js` — credits HUD, pack reveal overlay,
  collection album: a plain page script next to panel.js (client.ejs hunk).
  No engine coupling beyond the `window['tcg*']` API + the `tcg` key.
