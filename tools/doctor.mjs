...
//             3 structural problem (hunk overlap, marker gap, rev skew, EOL)
//
...
const git = (repo, cmd) => { try { return execSync(`git -C "${path.join(ROOT, repo)}" ${cmd}`, { encoding: 'utf-8' }).trim(); } catch { return null; } };
...
for (const [repo, pinned] of Object.entries(report.revs)) {
    ...
    report.revs[repo] = { pinned, head, match: head === pinned, modified_files: dirty };
    if (head && pinned && head !== pinned) {
        issues.push({ sev: 'drift', msg: `${repo}/ HEAD ${head.slice(0, 7)} != pinned ${pinned.slice(0, 7)} — hunks were generated against a different rev; run apply --check` });
    }
}
...
if (asJson) {
    ...
    for (const [repo, r] of Object.entries(report.revs)) {
        console.log(`  ${repo.padEnd(10)} pinned ${String(r.pinned).slice(0, 7)} | HEAD ${String(r.head || '?').slice(0, 7)} ${r.match ? '= ok' : '!= DRIFT CHECK NEEDED'} | ${r.modified_files} tracked files modified (the modded state)`);
    }
